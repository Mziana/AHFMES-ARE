...
| `DEBT-03` | Critical | Enkapsulasi DB: `evidence.py` & `registry.py` memanggil `_get_conn()` | DEFERRED (Target API EventStore) |
| `DEBT-04` | Medium | Duplikasi konstanta lifecycle `state_machine.py` ↔ `registry.py` | DEFERRED (Target `constants.py`) |
