# AHFMES ARE-0 — Legacy Authority Quarantine Policy V9
...
```text
BASE_POLICY = PROJECT_GOVERNANCE/ARE0/QUARANTINE/AHFMES_ARE_0_LEGACY_AUTHORITY_QUARANTINE_POLICY_V8.md
```

All V7 whole-blob quarantine and fail-closed rules carry forward. For the
generation-37 wave under STRUCTURAL_GENERATION_S1, this policy replaces the
literal post-S0 output set AND adds the Scope-and-discipline section below;
both changes are integral parts of this policy, not exceptions to it.
...
JQO_LOCAL = tenth literal path above (ARE0 wave ledger)
...
DATED DIARIES        = new dated sibling diary files are PROHIBITED during the
                       wave (other than JQO_LOCAL itself); both JQO surfaces
                       absorb all chronology.
